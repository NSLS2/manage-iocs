...
from manage_iocs.utils import get_ioc_statuses, find_installed_iocs
...
def test_state_change_commands(sample_iocs, ioc_name, command, before_state, before_enabled, after_state, after_enabled):
...
def test_status(sample_iocs, capsys):
    rc = cmds.status()
    captured = capsys.readouterr()
    expected_output = f"""IOC Status Auto-Start
----------------------------
